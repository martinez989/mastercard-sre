terraform {
  ...
  backend "s3" {
    bucket         = "eks-state-prod" # Unique S3 bucket for prod state
    key            = "eks/prod/terraform.tfstate"
    region         = "eu-central-1"
    encrypt        = true
    dynamodb_table = "eks-state-lock-prod" # Unique DynamoDB table for prod lock
  }
}
...
resource "aws_iam_role" "aws_lb_controller_irsa" {
  name        = "prod-aws-lb-controller-irsa"
...
resource "aws_iam_role" "cluster_autoscaler_irsa" {
  name        = "prod-cluster-autoscaler-irsa"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "cluster_autoscaler_policy" {
  ...
  description = "IAM policy for EKS Cluster Autoscaler in prod"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
